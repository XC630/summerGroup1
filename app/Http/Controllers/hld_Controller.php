...
use App\Model\hld_test;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use PhpParser\Node\Scalar\String_;

class hld_Controller extends Controller
{
    ...
    public function  SumR(Request $request){
        ...
        $res_zlc['C']=$resc;
        arsort($res_zlc);
        $hld= array_keys($res_zlc);

        $hld_zlc =$hld[0].$hld[1].$hld[2];
        $hld_zlc=(array)$hld_zlc;
        $hld_zlc_zlc=$hld_zlc[0];

        $result = hld_test::MyqCreateR($email,$req1,$req2,$res,$hld_zlc_zlc);

        $data2 = DB::table("hld_answer")->where("answer", "=", $hld_zlc)->select("details")->get();
